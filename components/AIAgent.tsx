    ...
    <div className="fixed bottom-8 right-8 z-[500]">
      {isOpen ? (
        <div className="bg-white w-[350px] h-[500px] rounded-[2.5rem] shadow-2xl border border-gray-100 flex flex-col overflow-hidden animate-in slide-in-from-bottom-4 duration-300">
          <div className="bg-[#2E5C4E] p-6 text-white flex justify-between items-center">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 bg-white/20 rounded-xl flex items-center justify-center backdrop-blur-md">
                <Bot size={24} />
              </div>
              <div>
                <h3 className="text-sm font-semibold">{t('ai_agent_title')}</h3>
                <p className="text-[8px] font-bold opacity-50">{t('ai_agent_status')}</p>
              </div>
            </div>
          ...
          <div ref={scrollRef} className="flex-grow p-6 overflow-y-auto space-y-4 bg-gray-50/50">
            {messages.length === 0 && (
              <div className="text-center py-10 space-y-4 opacity-30">
                ...
                <p className="text-[10px] font-semibold opacity-30">{t('ai_agent_placeholder')}</p>
              </div>
            )}
            {messages.map((msg, i) => (
              <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[80%] p-4 rounded-2xl text-xs font-medium leading-relaxed ${msg.role === 'user'
                  ? 'bg-[#2E5C4E] text-white rounded-tr-none'
            ...
            {loading && (
              <div className="flex justify-start">
                <div className="bg-white p-4 rounded-2xl rounded-tl-none shadow-sm border border-gray-100 flex gap-1">
                  <div className="w-1.5 h-1.5 bg-[#5B8C51] rounded-full animate-bounce"></div>
                  <div className="w-1.5 h-1.5 bg-[#5B8C51] rounded-full animate-bounce [animation-delay:0.2s]"></div>
                  <div className="w-1.5 h-1.5 bg-[#5B8C51] rounded-full animate-bounce [animation-delay:0.4s]"></div>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="p-4 bg-white border-t border-gray-100 flex gap-2">
            ...
            <button
              onClick={handleSend}
              disabled={loading}
              className="bg-[#2E5C4E] text-white p-3 rounded-xl hover:scale-105 active:scale-95 transition-all shadow-lg disabled:opacity-50"
            >
              <Send size={18} />
            </button>
          </div>
